public class SingerRunner {

    public static void main(String[] args) {

        Singer singer = new Singer();

        singer.singers = new String[50];

        singer.storeSinger("Arijit Singh");
        singer.storeSinger("Shreya Ghoshal");
        singer.storeSinger("Sonu Nigam");
        singer.storeSinger("Armaan Malik");
        singer.storeSinger("Sunidhi Chauhan");
        singer.storeSinger("KK");
        singer.storeSinger("Neha Kakkar");
        singer.storeSinger("Asha Bhosle");
        singer.storeSinger("Darshan Raval");
        singer.storeSinger("Alka Yagnik");

        singer.storeSinger("Lata Mangeshkar");
        singer.storeSinger("Mohit Chauhan");
        singer.storeSinger("Atif Aslam");
        singer.storeSinger("Kumar Sanu");
        singer.storeSinger("Jubin Nautiyal");
        singer.storeSinger("Shaan");
        singer.storeSinger("Honey Singh");
        singer.storeSinger("Diljit Dosanjh");
        singer.storeSinger("Sid Sriram");
        singer.storeSinger("Palak Muchhal");

        singer.storeSinger("Badshah");
        singer.storeSinger("Ankit Tiwari");
        singer.storeSinger("Vishal Dadlani");
        singer.storeSinger("Udit Narayan");
        singer.storeSinger("Arijit Ananya");

        singer.storeSinger("Rajesh Krishnan");
        singer.storeSinger("Vijay Prakash");
        singer.storeSinger("Nithi Mohan");
        singer.storeSinger("Chandrika Gururaj");
        singer.storeSinger("Anuradha Bhat");
        singer.storeSinger("C. Ashwath");
        singer.storeSinger("S. Janaki");
        singer.storeSinger("Archana Udupa");
        singer.storeSinger("Raghu Dixit");
        singer.storeSinger("P. B. Sreenivas");

        singer.storeSinger("Shankar Mahadevan");
        singer.storeSinger("Chinmayi");
        singer.storeSinger("Karthik");
        singer.storeSinger("SP Balasubrahmanyam");
        singer.storeSinger("Hariharan");

        singer.storeSinger("Shweta Mohan");
        singer.storeSinger("K. S. Chithra");
        singer.storeSinger("Yazin Nizar");
        singer.storeSinger("Haricharan");
        singer.storeSinger("Benny Dayal");

        singer.storeSinger("Anirudh Ravichander");
        singer.storeSinger("Puneeth Rajkumar");
        singer.storeSinger("Hemanth Kumar");
        singer.storeSinger("Ajay Warrier");
        singer.storeSinger("Naveen Sajju");
		
		boolean result = singer.searchSinger("Atif Aslam");

        System.out.println("Search Result: " + result);
    }
}